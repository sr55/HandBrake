...
static void write_chapter_names(hb_dict_t *job_dict, const char *marker_file);
...
hb_dict_t * PreparePreset(const char *preset_name)
{
    ...
    if (selection != NULL)
    {
        hb_dict_set(preset, "SubtitleTrackSelectionBehavior", hb_value_string(selection));
    }
